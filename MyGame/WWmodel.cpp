...
Model Model::Load(const char* path, uint32_t textureIndex) {
    Model model;
...
Model Model::LoadSkybox(const char* path, uint32_t textureIndex) {
    Model model;
...
void Model::translateModelMatrix(glm::vec3 v) {
...
void Model::rotateModelMatrix(float angle, glm::vec3 axis) {
    glm::mat4 transform = glm::rotate(modelMatrix, glm::radians(angle), axis);
    modelMatrix = transform * modelMatrix;
    boundingBox = computeBoundingBox(transformedVertices(vertices, modelMatrix));
}

std::vector<glm::vec3> Model::transformedVertices(const std::vector<Vertex> vertices, const glm::mat4& transform) {
...
void Model::createGraphicsPipeline(bool isNormalModel) {
...
std::vector<char> Model::readFile(const std::string& filename) {
...
VkShaderModule Model::createShaderModule(const std::vector<char>& code) {
