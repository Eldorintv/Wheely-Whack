...
bool BoundingBox::isInside(const glm::vec3& vert, float err) const {
...
BoundingBox::BoundingBox() : min(std::numeric_limits<float>::max()), max(std::numeric_limits<float>::lowest()) {}
